...
function Dashboard() {
  ...
  const submit = () => {
    if (!file) { setNotice('Choose a source file first.'); return; }
    setActiveView('workspace');
    setNotice('');
    createJob.mutate(
      { data: { file, preset, prompt: prompt.trim() || undefined } },
      {
        onSuccess: (created) => { setJobId(created.id); void recentJobs.refetch(); },
        onError: (error) => {
          const raw = error instanceof Error ? error.message : 'Upload could not start.';
